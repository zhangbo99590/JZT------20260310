    ...
    templates: [
      {
        ...
        instructions:
          "请按照模板格式填写企业基本信息、主要产品、核心技术、研发组织管理水平等内容。注意保持格式统一，确保信息准确完整。",
        examples: [
          "企业名称：深圳市创新科技有限公司",
          "主要产品：智能制造软件系统",
          "核心技术：人工智能算法、大数据分析",
      ...
      },
      {
        ...
        instructions:
          "请详细填写企业科技人员情况、研发投入、科技成果等信息，确保数据真实有效。",
        examples: ["科技人员占比：65%", "拥有知识产权：15件"],
    ...
    templates: [
      {
        ...
        instructions:
          "请按照企业会计准则编制完整的资产负债表、利润表、现金流量表，并提供详细的财务指标分析和说明。",
        examples: [
          "资产总计：5000万元",
          "营业收入：3000万元",
          "净利润：450万元",
    ...
    templates: [
      {
        ...
        instructions:
          "请详细描述企业的科技成果转化情况，包括转化项目、转化效果、产业化程度等。",
        examples: [
          "转化项目：AI智能识别系统",
          "转化周期：18个月",
          "产业化收入：1200万元",
...
class UnifiedTemplateService {
  ...
  searchTemplates(keyword: string): TemplateFile[] {
    ...
    // 使用统一的搜索工具
    return searchByKeyword(allTemplates, keyword, [
      "name",
      "description",
      "instructions",
    ]);
  ...
  getTemplateVersionHistory(templateId: string): TemplateVersionHistory | null {
    return (
      versionHistory.find((history) => history.templateId === templateId) ||
      null
    );
  }
  ...
  checkForUpdates(
    ...
    return {
      ...
      updateInfo: hasUpdate
        ? `发现新版本 ${latestVersion.version}，建议更新`
        : undefined,
    };
  }
  ...
  getLastValidationLog(): null | {
    ...
    try {
      const parsed = StorageUtils.getItem("template_validation_results", null);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (txtResponse.ok) {
          ...
          this.downloadBlob(
            blob,
            template.fileName.replace(/\.(docx|xlsx|pdf)$/, ".txt"),
          );
          return {
            success: true,
  ...
  private async validateTemplateFile(
    ...
    try {
      ...
      if (response.ok) {
        ...
        return {
          isValid: false,
          message: `文件不可访问 (HTTP ${response.status})`,
        };
      }
    } catch (error) {
      return {
