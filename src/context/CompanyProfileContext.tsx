  ...
  const updateProfile = (newProfile: Partial<CompanyProfile>) => {
    ...
    setTimeout(() => {
      setProfile((prev) => {
        ...
        // 基础信息 (5分)
        if (updated.companyName && updated.companyName !== "未认证企业")
          filledFields++;
        ...
        // 资质认证 (2分)
        if (updated.qualifications && updated.qualifications.length > 0)
          filledFields++;
        if (updated.certifications && updated.certifications.length > 0)
          filledFields++;

        // 动态画像 (3分)
        if (updated.behaviorTags && updated.behaviorTags.length > 0)
          filledFields++;
        if (updated.businessStatus && updated.businessStatus.length > 0)
          filledFields++;
        ...
        updated.completeness = Math.min(
          100,
          20 + Math.round((filledFields / 17) * 80),
        );
...
export const useCompanyProfileContext = () => {
  ...
  if (!context) {
    throw new Error(
      "useCompanyProfileContext must be used within a CompanyProfileProvider",
    );
  }
  return context;
};
