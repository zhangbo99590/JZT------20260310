...
const MyApplicationsOptimized: React.FC = () => {
  ...
  const [sortOrder, setSortOrder] = useState<
    "updateTime_desc" | "submitTime_desc" | "deadline_asc"
  >("updateTime_desc");
      ...
      if (value) {
        if (key === "department") {
          ...
          result = result.filter((item) =>
            item.policyType.includes(value.replace("类", "")),
          );
        }
        // 其他筛选条件可以根据实际需求添加
      }
      ...
      switch (sortOrder) {
        ...
        case "submitTime_desc":
          return (
            ...
            new Date(a.submitTime || 0).getTime()
          );
        case "deadline_asc":
          return (
  ...
  const getDeadlineInfo = (deadline: string, status: string) => {
    ...
    if (status === "expired" || daysLeft < 0) {
      ...
      return {
        text: deadlineDate.format("YYYY-MM-DD"),
        color: "#8c8c8c",
        urgent: false,
      };
    }
  ...
  return (
    <div
      className="my-applications-optimized"
      style={{
        padding: "24px",
        background: "#f5f5f5",
        minHeight: "100vh",
        ...
        <Col xs={24} md={18} lg={19}>
          ...
          >
            ...
            >
              ...
              <Row
                gutter={[16, 16]}
                style={{ marginBottom: filterCollapsed ? 0 : 16 }}
              >
            ...
            >
              <Space wrap>
                <Checkbox
                  ...
                  onChange={(e) =>
                    setSelectedRowKeys(
                      e.target.checked ? filteredData.map((i) => i.id) : [],
                    )
                  }
                >
                  全选
            ...
            <Row gutter={[16, 16]}>
              {paginatedData.map((item) => {
                ...
                const isExpired =
                  item.status === "expired" || deadlineInfo.text === "已截止";
                    ...
                    >
                        ...
                        <Alert
                          ...
                          message={
                            <div>
                              ...
                              <div>
                                需补正: {item.missingMaterials?.join("、")}
                              </div>
                            </div>
                          }
                        />
                      ...
                      >
                        ...
                        {isExpired ? (
                          ...
                          <>
                            {["draft", "to_submit", "needs_revision"].includes(
                              item.status,
                            ) && (
                              <Button
                                type="primary"
              ...
              >
                <Pagination
                  ...
                  showTotal={(total, range) =>
                    `共 ${total} 条，当前 ${range[0]}-${range[1]} 条`
                  }
