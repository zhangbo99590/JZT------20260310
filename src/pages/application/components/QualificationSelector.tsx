...
const QualificationSelector: React.FC<QualificationSelectorProps> = ({
  ...
  const filteredGroups = useMemo(() => {
    ...
    return QUALIFICATION_DATA.map((group) => ({
      ...group,
      qualifications: group.qualifications.filter(
        (q) =>
  ...
  const handleGroupAction = (
    ...
    if (action === "selectAll") {
      ...
      newValue = value
        .filter((v) => !groupValues.includes(v))
        .concat(unselectedInGroup);
    }
          ...
          title={
            <div>
              ...
              <div style={{ marginBottom: 4 }}>
                <strong>适用行业：</strong>
                {qualification.applicableIndustries.join("、")}
              </div>
              <div style={{ marginBottom: 4 }}>
                <strong>申报条件：</strong>
          ...
          >
            <Space>
              ...
              <span
                style={{
                  fontWeight:
                    qualification.level === "national" ? "bold" : "normal",
                }}
              >
                {qualification.label}
              </span>
