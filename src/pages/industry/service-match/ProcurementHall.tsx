...
const ProcurementHall: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const type = 'demand'; // Fixed for Procurement Hall
        ...
        const mappedData = listData.map((p: any) => ({
            ...
            quantity: p.details?.quantity ? `${p.details.quantity}${p.details.quantityUnit || ''}` : '-',
  ...
  const handleBatchExport = () => {
    ...
    try {
        ...
        link.href = url;
        link.download = `采购大厅_批量导出_${new Date().toISOString().slice(0,10)}.csv`;
            ...
            <Empty 
              ...
              description={
                <div style={{ textAlign: 'center' }}>
                  <Text type="secondary">暂无相关需求信息</Text>
