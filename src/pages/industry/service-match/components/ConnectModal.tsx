...
const ConnectModal: React.FC<ConnectModalProps> = ({ open, onCancel, target }) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (isBatch) {
         ...
         await Promise.all(target.map((t: any) => 
            createConnection(t.id, values.description, applicationId)
         ));
      } else {
         await createConnection(singleTarget.id, values.description, applicationId);
      }
  ...
  const renderForm = () => (
    <>
      ...
      <div style={{ 
        ...
        {isBatch ? (
          <Space align="center">
            ...
            <div>
              <Text strong style={{ fontSize: '16px' }}>批量对接申请</Text>
              <div style={{ fontSize: '12px', color: '#666' }}>
                即将向 <Text strong style={{ color: THEME.primary }}>{target.length}</Text> 家企业发送对接申请
              </div>
            </div>
          </Space>
        ) : (
          <Row gutter={16} align="middle">
            <Col flex="auto">
              <Title level={5} style={{ margin: 0, marginBottom: 4 }}>{singleTarget?.name || '目标企业'}</Title>
              <Space size={12} style={{ fontSize: '12px', color: '#666' }}>
                <span><SafetyCertificateFilled style={{ color: '#52c41a' }} /> 已认证</span>
            ...
            <Col flex="80px" style={{ textAlign: 'center' }}>
              <Progress 
                ...
                format={percent => <span style={{ fontSize: '12px', color: '#333' }}>{percent}%</span>}
              />
              <div style={{ fontSize: '10px', marginTop: 4, color: '#999' }}>匹配度</div>
            </Col>
          </Row>
        )}
      </div>
      ...
      <Form
        form={form}
        layout="vertical"
        initialValues={{ budgetUnit: 'wan' }}
      >
        {/* Basic Info */}
        <Title level={5} style={{ fontSize: '14px', marginBottom: '16px', borderLeft: `3px solid ${THEME.primary}`, paddingLeft: '8px' }}>基础信息</Title>
        <Row gutter={16}>
          <Col span={12}>
             <Form.Item label="申请编号">
               <Input value={applicationId} disabled style={{ color: '#666', background: '#f5f5f5' }} />
             </Form.Item>
          </Col>
          <Col span={12}>
             <Form.Item label="申请企业">
               <Input value="杭州示例科技有限公司" disabled suffix={<a href="#">修改</a>} />
             </Form.Item>
          </Col>
        </Row>
        <Row gutter={16}>
        ...
        <Title level={5} style={{ fontSize: '14px', marginBottom: '16px', marginTop: '8px', borderLeft: `3px solid ${THEME.primary}`, paddingLeft: '8px' }}>对接需求</Title>
        <Form.Item 
          label="对接目的" 
          name="purpose" 
        ...
        <Row gutter={16}>
          <Col span={16}>
            <Form.Item label="预期预算 (选填)" name="budgetAmount" style={{ marginBottom: 0 }}>
               <Input.Group compact>
                 <Form.Item name="budgetAmount" noStyle>
                   <Input style={{ width: '70%' }} placeholder="请输入数字" type="number" />
                 </Form.Item>
  ...
  const renderSuccess = () => (
    <div style={{ padding: '20px 0' }}>
      <Result
        ...
        title="申请提交成功！"
        subTitle={
          <div style={{ textAlign: 'left', maxWidth: '400px', margin: '0 auto', background: '#f9f9f9', padding: '16px', borderRadius: '8px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
              ...
              <Text>{isBatch ? `批量对接 ${target.length} 家企业` : singleTarget?.name}</Text>
            </div>
      ...
      <div style={{ marginTop: '20px', padding: '0 40px' }}>
        <Title level={5} style={{ fontSize: '14px', marginBottom: '20px', textAlign: 'center' }}>处理进度跟踪</Title>
        <Steps size="small" current={1}>
          <Steps.Step title="提交申请" description="已完成" />
          <Steps.Step title="智能匹配" description="进行中" icon={<LoadingIcon />} />
      ...
      <div style={{ display: 'flex', alignItems: 'center', background: '#f0f5ff', padding: '12px', borderRadius: '8px' }}>
        <Avatar src="https://api.dicebear.com/7.x/avataaars/svg?seed=Felix" size={48} style={{ marginRight: '16px' }} />
        <div style={{ flex: 1 }}>
          <div style={{ fontWeight: 'bold', marginBottom: '4px' }}>专属对接员：李经理</div>
          <div style={{ fontSize: '12px', color: '#666' }}>电话：138-0013-8000 (工作日 9:00-18:00)</div>
        </div>
        <Button type="link" size="small">联系对接员</Button>
      </div>
    </div>
  );
