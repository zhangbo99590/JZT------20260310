...
const ServiceMatchCard: React.FC<ServiceMatchCardProps> = ({ 
  ...
  return (
    ...
    >
      {/* Selection Checkbox */}
      <div 
        style={{ position: 'absolute', top: '50%', left: '15px', transform: 'translateY(-50%)', zIndex: 2 }}
      ...
      {/* Quality Score */}
      <div style={{ position: 'absolute', top: '15px', right: '15px', zIndex: 1 }}>
        <Tooltip title={`质量评分: ${item.score}星`}>
          <Rate 
            disabled 
            defaultValue={item.score} 
            character={<StarFilled style={{ fontSize: '14px' }} />} 
            style={{ color: item.score >= 4 ? '#faad14' : '#ccc', fontSize: '14px' }} 
          />
        </Tooltip>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', marginBottom: '12px', flexWrap: 'wrap', paddingRight: '100px' }}>
            ...
            <Text style={{ marginLeft: '12px', fontSize: '12px', color: THEME.textHint }}>
              {item.updateTime}
            </Text>
          </div>
          ...
          >
            ...
            {activeTab === 'procurement' && (
              <div style={{ marginTop: '8px', fontSize: '12px', color: THEME.textBody, display: 'flex', gap: '16px' }}>
                 <span>预算：<Text strong>{item.budget}</Text></span>
                 <span>数量：<Text strong>{item.quantity}</Text></span>
                 <span>截止：<Text type="danger">{item.deadline}</Text></span>
              </div>
            )}
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Space size={16} style={{ fontSize: '12px', color: THEME.textBody }}>
