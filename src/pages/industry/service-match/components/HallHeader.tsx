...
import { Input, Button, Row, Col, Dropdown, Space, Select, DatePicker, Card } from 'antd';
import { SearchOutlined, FilterOutlined, PlusOutlined, CloseCircleOutlined } from '@ant-design/icons';
  ...
  const filterMenu = (
    ...
    >
      <Space direction="vertical" style={{ width: '100%' }} size="middle">
        <div>
          <div style={{ marginBottom: 8, color: THEME.textBody }}>{isProcurement ? '采购品类' : '产品类别'}</div>
          <Select
            style={{ width: '100%' }}
            placeholder="请选择"
            allowClear
        ...
        <div>
          <div style={{ marginBottom: 8, color: THEME.textBody }}>{isProcurement ? '采购需求' : '需求分类'}</div>
          <Select
            style={{ width: '100%' }}
            placeholder="请选择"
            allowClear
        ...
        {isProcurement ? (
           <>
             <div>
              <div style={{ marginBottom: 8, color: THEME.textBody }}>采购数量</div>
               <Select
                style={{ width: '100%' }}
                placeholder="请选择"
                allowClear
             ...
             <div>
              <div style={{ marginBottom: 8, color: THEME.textBody }}>采购预算</div>
               <Select
                style={{ width: '100%' }}
                placeholder="请选择"
                allowClear
             ...
             <div>
               <div style={{ marginBottom: 8, color: THEME.textBody }}>截止日期</div>
               <DatePicker 
                 style={{ width: '100%' }} 
                 onChange={(date, dateString) => handleFilterChange('deadline', dateString)}
               />
             </div>
           </>
        ) : (
           <div>
            <div style={{ marginBottom: 8, color: THEME.textBody }}>报价区间</div>
             <Select
              style={{ width: '100%' }}
              placeholder="请选择"
              allowClear
        ...
        {!isProcurement && (
          <div>
            <div style={{ marginBottom: 8, color: THEME.textBody }}>发布日期</div>
            <DatePicker 
              style={{ width: '100%' }} 
    ...
    <div style={{ 
      ...
      <Row gutter={16} align="middle">
        ...
        <Col span={9} style={{ flex: '0 0 40%', maxWidth: '40%' }}>
          <Input
            placeholder={isProcurement ? "搜索采购企业 / 采购品类 / 采购需求" : "搜索企业名称 / 业务类型 / 产品名称"}
        ...
        <Col span={10} style={{ flex: '0 0 40%', maxWidth: '40%' }}>
          ...
          >
            <Button block style={{ textAlign: 'left', color: THEME.textBody }}>
              <FilterOutlined /> 筛选
              <span style={{ float: 'right', fontSize: '12px', color: THEME.textHint }}>
                {filterOpen ? '收起' : '展开'}
              </span>
            </Button>
          </Dropdown>
