...
import { createPublication, getPublicationById, updatePublication } from '../../../services/industryService';
...
const ServicePublish: React.FC = () => {
  ...
  const onScenarioChange = (e: any) => {
    ...
    form.resetFields(['category', 'title', 'description', 'supplyType', 'spec', 'priceRange', 'coopMode', 'procurementCategory', 'quantity', 'budget', 'deadline', 'inventory', 'serviceCycle', 'deliveryMode', 'resourceType', 'availableTime']);
  };
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      ...
      if (id) {
        ...
        message.success({ content: '更新成功，即将跳转至我的信息页', duration: 3 });
      } else {
        await createPublication(pubData as NewPublicationForm);
        message.success({ content: '提交成功，即将跳转至我的信息页', duration: 3 });
      }
        ...
        <>
          ...
          >
            <Radio.Group buttonStyle="solid">
               <Radio.Button value="product" style={{ color: '#1890ff' }}>产品</Radio.Button>
               <Radio.Button value="service" style={{ color: '#52c41a' }}>服务</Radio.Button>
               <Radio.Button value="resource" style={{ color: '#fa8c16' }}>资源</Radio.Button>
            </Radio.Group>
          </Form.Item>
          ...
          >
            {({ getFieldValue }) => {
              ...
              return (
                <div style={{ background: '#f9f9f9', padding: '16px', borderRadius: '4px', marginBottom: '24px' }}>
                    {type === 'product' && (
                        <Row gutter={24}>
                            <Col span={12}>
                                <Form.Item label="规格/属性" name="spec" rules={[{ required: true, message: '请输入规格' }]} tooltip="如：500g/盒，S/M/L">
                                    <Input placeholder="请输入规格属性" />
                                </Form.Item>
                            </Col>
                            <Col span={12}>
                                <Form.Item label="报价区间" name="priceRange" rules={[{ required: true, message: '请输入报价' }]} tooltip="如：100-200元/件">
                                    <Input placeholder="请输入报价区间" />
                                </Form.Item>
                            </Col>
                            <Col span={12}>
                                <Form.Item label="库存数量" name="inventory" rules={[{ required: true, message: '请输入库存' }]}>
                    ...
                    {type === 'service' && (
                        <Row gutter={24}>
                            <Col span={12}>
                                <Form.Item label="服务周期" name="serviceCycle" rules={[{ required: true, message: '请输入周期' }]} tooltip="如：1年，3个月">
                                    <Input placeholder="请输入服务周期" />
                                </Form.Item>
                            </Col>
                            <Col span={12}>
                                <Form.Item label="交付方式" name="deliveryMode" rules={[{ required: true, message: '请选择交付方式' }]}>
                            ...
                            <Col span={12}>
                                <Form.Item label="服务价格" name="priceRange" rules={[{ required: true, message: '请输入价格' }]}>
                    ...
                    {type === 'resource' && (
                        <Row gutter={24}>
                            <Col span={12}>
                                <Form.Item label="资源类型" name="resourceType" rules={[{ required: true, message: '请输入类型' }]}>
                                    <Input placeholder="如：厂房、设备、数据" />
                                </Form.Item>
                            </Col>
                            <Col span={12}>
                                <Form.Item label="可用时间" name="availableTime" rules={[{ required: true, message: '请选择时间' }]}>
                                    <DatePicker.RangePicker style={{ width: '100%' }} />
                                </Form.Item>
                            </Col>
                            <Col span={12}>
                                <Form.Item label="合作模式" name="coopMode" rules={[{ required: true, message: '请选择模式' }]}>
                                    <Select placeholder="请选择">
    ...
    <div style={COMMON_STYLES.pageContainer}>
      <Space direction="vertical" style={{ width: '100%' }} size={24}>
        ...
        <div style={{ display: 'flex', alignItems: 'center' }}>
          ...
          />
          <Title level={4} style={{ margin: 0, ...COMMON_STYLES.title }}>{id ? '编辑信息' : '发布信息'}</Title>
        </div>

        {/* Top Scenario Switch & Quick Actions */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
             <div style={{ background: '#f0f0f0', borderRadius: '30px', padding: '4px', display: 'inline-flex' }}>
                 <div 
                    ...
                    style={{
                        ...
                        borderRadius: '24px',
                        cursor: 'pointer',
                        background: scenarioType === 'supply' ? THEME.primary : 'transparent',
                 ...
                 <div 
                    ...
                    style={{
                        ...
                        borderRadius: '24px',
                        cursor: 'pointer',
                        background: scenarioType === 'demand' ? THEME.primary : 'transparent',
        ...
        <div style={COMMON_STYLES.card}>
          ...
          >
            {/* 1. Basic Info */}
            <Title level={5} style={{ fontSize: '16px', marginBottom: '24px', borderLeft: `4px solid ${THEME.primary}`, paddingLeft: '12px' }}>基础信息</Title>
            
            <Row gutter={24}>
                <Col span={12}>
                    <Form.Item label="企业名称" required tooltip="系统自动拉取，不可修改">
                        <div style={{ display: 'flex', justifyContent: 'space-between', background: '#f5f5f5', padding: '5px 11px', border: '1px solid #d9d9d9', borderRadius: '2px' }}>
                            <span style={{ color: '#999' }}>{enterpriseName}</span>
                            <a onClick={() => message.info('跳转至企业中心')} style={{ fontSize: '12px' }}>前往企业中心 &gt;</a>
                        </div>
                    </Form.Item>
                </Col>
                <Col span={12}>
                     <Form.Item label="所在地区" required tooltip="系统自动拉取，不可修改">
                        <Input value={enterpriseRegion} disabled style={{ color: '#999', background: '#f5f5f5' }} />
                     </Form.Item>
                </Col>
            </Row>
            ...
            {/* 2. Core Info */}
            <Title level={5} style={{ fontSize: '16px', marginBottom: '24px', borderLeft: `4px solid ${THEME.primary}`, paddingLeft: '12px' }}>
            ...
            <Title level={5} style={{ fontSize: '16px', marginBottom: '24px', borderLeft: `4px solid ${THEME.primary}`, paddingLeft: '12px' }}>补充信息</Title>
            ...
            <div style={{ marginTop: '32px', display: 'flex', gap: '16px', justifyContent: 'center' }}>
              <Button size="large" onClick={onSaveDraft} style={{ width: '160px' }}>保存草稿</Button>
              <Button 
                type="primary" 
                size="large" 
                htmlType="submit" 
