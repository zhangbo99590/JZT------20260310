...
const convertedMockPolicies: EnhancedPolicyData[] = mockPolicies.map(p => ({
  ...
  keywords: [...p.tags, ...segmentKeyword(p.title), p.district, p.publishOrg || ''].filter(Boolean),
...
const originalPolicyDatabase: EnhancedPolicyData[] = [
  {
    ...
    tags: [
      { text: '立项公示', type: 'announcement', color: '#1890ff', bgColor: '#e6f7ff' },
  ...
  },
  {
    ...
    tags: [
      { text: '资金支持', type: 'category', color: '#722ed1', bgColor: '#f9f0ff' },
      { text: '区级', type: 'level', color: '#faad14', bgColor: '#fffbe6' }
    ],
  ...
  },
  {
    ...
    tags: [
      { text: '产业扶持', type: 'category', color: '#13c2c2', bgColor: '#e6fffb' },
      { text: '区级', type: 'level', color: '#faad14', bgColor: '#fffbe6' }
    ],
  ...
  },
  {
    ...
    tags: [
      { text: '试点项目', type: 'category', color: '#eb2f96', bgColor: '#fff0f6' },
      { text: '区级', type: 'level', color: '#faad14', bgColor: '#fffbe6' }
    ],
  ...
  },
  {
    ...
    tags: [
      { text: '企业认定', type: 'category', color: '#fa8c16', bgColor: '#fff7e6' },
      { text: '区级', type: 'level', color: '#faad14', bgColor: '#fffbe6' }
    ],
  ...
  },
  {
    ...
    tags: [
      { text: '招商引资', type: 'category', color: '#2f54eb', bgColor: '#f0f5ff' },
      { text: '区级', type: 'level', color: '#faad14', bgColor: '#fffbe6' }
    ],
  ...
  },
  {
    ...
    tags: [
      { text: '申报指南', type: 'category', color: '#13c2c2', bgColor: '#e6fffb' },
      { text: '区级', type: 'level', color: '#faad14', bgColor: '#fffbe6' }
    ],
  ...
  },
  {
    ...
    tags: [
      { text: '补贴办法', type: 'category', color: '#722ed1', bgColor: '#f9f0ff' },
      { text: '区级', type: 'level', color: '#faad14', bgColor: '#fffbe6' }
    ],
...
export const searchEnhancedPoliciesLegacy = (
    ...
    results = results.filter(policy => {
      ...
      return coreTerms.some(term => {
        ...
        return (
          ...
          policy.keywords.some(k => k.toLowerCase().includes(normalizedTerm)) ||
          policy.publishOrg.toLowerCase().includes(normalizedTerm) ||
          policy.applicableIndustries.some(industry => industry.toLowerCase().includes(normalizedTerm)) ||
  ...
  if (districts && districts.length > 0) {
    results = results.filter(policy => {
      return districts.some(district => {
        ...
        const normalizedPolicyDistrict = policy.district.replace('区', '').replace('市', '');
        return (
    ...
    results = results.filter(policy => {
      return policy.applicableIndustries.some(policyIndustry => {
        return industries.some(filterIndustry => {
          ...
          const keywords = filterIndustry.split(/\/|、|，| /).filter(k => k.trim().length > 1);
          
          // 1. 直接包含匹配（不区分大小写）
          if (
            policyIndustry.toLowerCase().includes(filterIndustry.toLowerCase()) || 
          ...
          return keywords.some(k => {
            ...
            return normalizedIndustry.includes(normalizedKeyword) || normalizedKeyword.includes(normalizedIndustry);
          });
        });
      });
    });
  ...
  if (categories && categories.length > 0) {
    results = results.filter(policy => 
      categories.includes(policy.category)
    );
  }
