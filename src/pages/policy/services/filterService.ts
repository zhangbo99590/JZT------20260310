...
class FilterService {
  ...
  public addCondition(condition: Omit<FilterCondition, 'id'>): string {
    ...
    try {
      ...
      // 触发事件
      this.emitEvent(FilterEventType.CONDITION_ADDED, { condition: newCondition });
  ...
  public removeCondition(conditionId: string): boolean {
    ...
    try {
      const index = this.filterState.conditions.findIndex(c => c.id === conditionId);
      
      if (index >= 0) {
        const removedCondition = this.filterState.conditions.splice(index, 1)[0];
        this.filterState.lastUpdated = Date.now();
        
        // 触发事件
        this.emitEvent(FilterEventType.CONDITION_REMOVED, { condition: removedCondition });
   ...
   * 更新筛选条件
   */
  public updateCondition(conditionId: string, updates: Partial<FilterCondition>): boolean {
    const startTime = performance.now();
   ...
   * 生成分面统计
   */
  private generateFacets(allPolicies: PolicyItem[], filteredPolicies: PolicyItem[]): any[] {
    // 这里可以实现分面统计逻辑
    return [];
  }
   ...
   * 更新性能指标
   */
  private updatePerformanceMetrics(metric: keyof FilterPerformanceMetrics, value: number): void {
