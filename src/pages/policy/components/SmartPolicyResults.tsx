...
const SmartPolicyResults: React.FC<SmartPolicyResultsProps> = ({
  ...
  const getStatusTag = (status: string) => {
    switch (status) {
      case "active":
        return <Tag color="success">申报中</Tag>;
      case "upcoming":
        return <Tag color="warning">即将开始</Tag>;
      case "closed":
  ...
  const tabs = [
    {
      label: `政策文件 (${Math.floor(total * 0.7)})`,
      key: "policy",
    ...
    <div className="smart-policy-results">
      ...
      <Card bordered={false} bodyStyle={{ padding: "0 24px 24px" }}>
        ...
        <List
          ...
          locale={{
            emptyText: (
              <Empty description="未找到匹配政策，请调整筛选条件或更换搜索关键词" />
            ),
          }}
          renderItem={(item) => (
            <List.Item
            ...
            >
              <div style={{ display: "flex", alignItems: "flex-start" }}>
                <div style={{ flex: 1 }}>
                  ...
                  >
                    <span>
                      <CalendarOutlined /> 发布日期：{item.date}
                    </span>
                    <span>
                      <AppstoreOutlined /> 适用行业：{item.industry.join("、")}
                    </span>
                    <span>
                      <BankOutlined /> 发文机构：
                      <HighlightText
