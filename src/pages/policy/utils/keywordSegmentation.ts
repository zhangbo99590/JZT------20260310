...
export const segmentKeyword = (keyword: string): string[] => {
  ...
  const districts = [
    ...
    '怀柔区', '平谷区', '密云区', '延庆区', '北京市', '北京'
  ];
  
  // 常见的行业/领域词
  const industries = [
    '金融', '科技', '文化', '教育', '医疗', '制造', '服务',
    '建筑', '房地产', '农业', '商业', '贸易', '物流', '旅游',
    '互联网', '软件', '硬件', '电子', '通信', '能源', '环保',
    '交通', '运输', '餐饮', '零售', '批发', '创业', '创新'
  ];
  
  // 常见的政策类型词
  const policyTypes = [
    '补贴', '资助', '奖励', '扶持', '优惠', '减免', '贷款',
    '融资', '投资', '税收', '人才', '项目', '认定', '申报',
    '评审', '公示', '政策', '办法', '通知', '指南', '条例'
  ];
 ...
 */
export const formatKeywordDisplay = (keyword: string): Array<{text: string, isCore: boolean}> => {
  if (!keyword || !keyword.trim()) return [];
