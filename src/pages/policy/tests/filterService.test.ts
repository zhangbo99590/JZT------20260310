...
const generateMockPolicies = (count: number): PolicyItem[] => {
  ...
  return Array.from({ length: count }, (_, index) => ({
    ...
    publishDate: new Date(2024, 0, index % 30 + 1).toISOString().split('T')[0],
...
describe('FilterService', () => {
  ...
  describe('筛选逻辑测试', () => {
    test('应该正确筛选单个条件', async () => {
      ...
      expect(result.items.every(item => item.district === '朝阳区')).toBe(true);
    });
  ...
  describe('性能测试', () => {
    ...
    test('复杂筛选条件性能测试', async () => {
      ...
      expect(result.items.every(item => 
        ...
        ['科技、教育 / 科技', '工业、交通 / 信息产业（含电信）'].includes(item.industry) &&
