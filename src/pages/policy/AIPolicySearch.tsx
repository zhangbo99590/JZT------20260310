...
import { Layout, Input, Button, Typography, Row, Col, Space, message, Spin, Alert, Tabs } from 'antd';
import { SearchOutlined, ClockCircleOutlined, InfoCircleOutlined, RobotOutlined } from '@ant-design/icons';
...
const AIPolicySearch: React.FC = () => {
  ...
  const [searchHistory, setSearchHistory] = useState(['高新技术', '专精特新', '研发补贴']);
  ...
  const performSearch = () => {
    ...
    setTimeout(() => {
      ...
      if (filters.industry.length > 0) {
        filtered = filtered.filter(p => filters.industry.some(ind => p.industry.includes(ind)));
      }
      ...
      // Feedback generation
      const regionText = filters.region.length > 0 ? filters.region.join('、') : '';
      const keyText = keyword ? `"${keyword}"` : '';
      
      if (filtered.length > 0) {
        setFeedback(`搜索完成，找到 ${filtered.length} 条 ${keyText} ${regionText ? `(${regionText})` : ''} 相关政策`);
      } else {
        setFeedback('');
      }
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '24px' }}>
        {activeTab === 'match' && (
          <div style={{ textAlign: 'center', marginBottom: 24 }}>
            <Typography.Title level={2} style={{ margin: '0 0 8px 0' }}>项目列表</Typography.Title>
          ...
          <>
            ...
            <div style={{ textAlign: 'center', marginBottom: 32 }}>
              <Input.Search
                ...
                enterButton={<Button type="primary" size="large" style={{ height: 40 }}>立即搜索</Button>}
                size="large"
                value={keyword}
              ...
              <div style={{ marginTop: 12, color: '#666', fontSize: 13 }}>
                <Space split={<span style={{ color: '#ddd' }}>|</span>}>
                  <span><InfoCircleOutlined style={{ marginRight: 4 }} /> 主动检索政策：输入关键词快速查找</span>
                  {searchHistory.length > 0 && (
                    <Space>
                      ...
                      {searchHistory.map(h => (
                        <span 
                          key={h} 
                          onClick={() => { setKeyword(h); handleSearchClick(); }} 
            ...
            <SmartPolicyFilter 
              ...
              onReset={() => handleFilterChange({ region: [], industry: [], level: [], agency: [], subsidyType: [] })} 
            />
