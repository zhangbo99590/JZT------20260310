...
const TimelinessManagement: React.FC = () => {
  ...
  // 模拟法规状态数据
  const mockRegulationData: RegulationStatus[] = [
    {
      id: '1',
  ...
  const columns: ColumnsType<RegulationStatus> = [
        ...
        <div>
          ...
          {record.changeType && (
            <div style={{ marginTop: 4 }}>
              <Tag 
                color={
                  record.changeType === 'new' ? 'green' : 
                  record.changeType === 'modified' ? 'orange' : 'red'
                }
              >
                {record.changeType === 'new' ? '新增' : 
                 record.changeType === 'modified' ? '已修订' : '已废止'}
              </Tag>
            </div>
          )}
        </div>
    ...
    },
    {
      ...
      render: (status: string) => {
        const statusConfig = {
          effective: { color: 'success', text: '有效', icon: <CheckCircleOutlined /> },
          revised: { color: 'warning', text: '已修订', icon: <AlertOutlined /> },
          abolished: { color: 'error', text: '已废止', icon: <ExclamationCircleOutlined /> },
          pending: { color: 'processing', text: '待生效', icon: <ClockCircleOutlined /> }
        };
      ...
      render: (date: string) => {
        ...
        return (
          <span style={{ color: isNearDue ? '#ff4d4f' : undefined }}>
            {date}
            {isNearDue && <AlertOutlined style={{ marginLeft: 4, color: '#ff4d4f' }} />}
          </span>
        );
      }
    },
          ...
          <Col xs={24} sm={12} md={6} key={index}>
            <Card>
              <Statistic
                ...
                value={stat.value}
                valueStyle={{ 
                  color: stat.status === 'success' ? '#52c41a' : 
                         stat.status === 'warning' ? '#fa8c16' : 
                         stat.status === 'error' ? '#ff4d4f' : '#1890ff' 
                }}
                prefix={
                  stat.status === 'success' ? <CheckCircleOutlined /> :
                  stat.status === 'warning' ? <AlertOutlined /> :
                  stat.status === 'error' ? <ExclamationCircleOutlined /> :
                  <ClockCircleOutlined />
                }
              />
            </Card>
                    ...
                    <Timeline>
                      {recentChanges.map((change) => (
                        <Timeline.Item
                          key={change.id}
                          color={
                            change.changeType === 'new' ? 'green' :
                            change.changeType === 'modified' ? 'orange' : 'red'
                          }
                          dot={
                            change.changeType === 'new' ? <PlusOutlined /> :
                            change.changeType === 'modified' ? <EditOutlined /> :
                            <DeleteOutlined />
                          }
                        >
                          <div>
                            ...
                            <Tag 
                              color={
                                change.impactLevel === 'high' ? 'red' :
                                change.impactLevel === 'medium' ? 'orange' : 'green'
                              }
                              style={{ marginLeft: 8 }}
                            >
                              {change.impactLevel === 'high' ? '高影响' :
                               change.impactLevel === 'medium' ? '中影响' : '低影响'}
                            </Tag>
                            <div style={{ marginTop: 4 }}>
                ...
                <List.Item>
                  <List.Item.Meta
                    avatar={
                      <Avatar 
                        icon={
                          item.changeType === 'new' ? <PlusOutlined /> :
                          item.changeType === 'modified' ? <EditOutlined /> :
                          <DeleteOutlined />
                        }
                        style={{ 
                          backgroundColor: 
                            item.changeType === 'new' ? '#52c41a' :
                            item.changeType === 'modified' ? '#fa8c16' : '#ff4d4f'
                        }}
                      />
                    }
                    title={item.title}
