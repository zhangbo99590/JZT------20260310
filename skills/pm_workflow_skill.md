...
### 4.1 原型规范
**1. 必须遵循 Ben Shneiderman 的“界面设计八项黄金法则”：**
   - **保持一致性 (Strive for consistency)**：相似操作使用相同的术语、颜色和布局。
   - **满足普遍可用性 (Seek universal usability)**：考虑不同用户水平，提供必要的引导或快捷方式。
   - **提供信息反馈 (Offer informative feedback)**：每个系统操作都应有适度的视觉或文本反馈。
   - **设计产生闭环的对话 (Design dialogs to yield closure)**：操作序列应有明确的起点、中间过程和终点提示。
   - **预防错误 (Prevent errors)**：设计防呆机制，提供简单、建设性的错误恢复指导。
   - **允许轻松撤销操作 (Permit easy reversal of actions)**：减轻用户焦虑，鼓励探索。
   - **支持内部控制点 (Keep users in control)**：让用户感觉自己是界面的掌控者。
   - **减轻短期记忆负担 (Reduce short-term memory load)**：界面保持简洁，必要信息在上下文中可见。
**2. 技术实现规范：**
   - 产出 **单文件 HTML 原型**，包含完整的 CSS 样式。
   - 强制使用 **Tailwind CSS**，并采用现代、简洁的设计风格。
   - 必须包含关键的交互状态（如：默认页、展开弹窗、成功提示等）。可以通过简单的原生 JavaScript 或 URL Hash (`#page1`) 来实现页面切换。
   - **沙盒锁定支持 (Focus Mode)**：在编写原生 JavaScript 时，必须加入 URL 参数解析逻辑（如 `?focus=feature_id`）。当处于 focus 模式时，锁定或遮罩其他无关的交互区域（如设置 `pointer-events: none; opacity: 0.5`），仅允许用户操作对应的功能点。这为了后续在 PRD 嵌入原型时，实现“仅展示对应功能交互”的需求。
